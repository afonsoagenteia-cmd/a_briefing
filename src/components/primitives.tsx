...
export function Logo({
  ...
  return (
    <span className="inline-flex select-none items-center gap-2.5">
      {src ? (
        <span
          className={`flex items-center justify-center overflow-hidden rounded-lg border-2 px-2 ${
            dark ? "border-paper/25 bg-paper" : "border-ink/20 bg-card"
          }`}
        ...
        >
          <img
            src={src}
            alt="Logotipo"
            className="h-full w-auto max-w-[160px] object-contain py-1"
            style={{ maxHeight: size + 2 }}
          />
        </span>
      ) : (
        <span
          className={`noise flex shrink-0 items-center justify-center rounded-lg border-2 shadow-hard-sm ${
            dark ? "border-paper/25 bg-pine" : "border-ink bg-ink"
          }`}
          style={{ width: size + 10, height: size + 10 }}
        >
          <svg viewBox="0 0 48 48" width={size} height={size} role="img" aria-label="RB·360 — Rebranding 360 graus">
            <path
              d="M40.6 30.9A17 17 0 1 1 38.5 15"
              fill="none"
              stroke="#5b8cff"
              strokeWidth="3"
              strokeLinecap="round"
            ...
            />
            <path
              d="M17 33V15.5h7.2a4.6 4.6 0 0 1 0 9.2H17m5.5 0L30 33"
              fill="none"
              stroke="#eef1f6"
              strokeWidth="3.2"
              strokeLinecap="round"
      ...
      {caption && (
        <span className="flex flex-col leading-none">
          <span className={`font-display text-lg font-extrabold tracking-tight ${dark ? "text-paper" : "text-ink"}`}>
            RB·360
          </span>
          <span className={`mt-1 font-mono text-[8px] font-bold uppercase tracking-[0.2em] ${dark ? "text-paper/45" : "text-ink/45"}`}>
            rebranding 360º
          </span>
        </span>
      )}
